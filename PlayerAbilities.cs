...
public class PlayerAbilities : MonoBehaviour
{
    ...
    //Jump Checks
        ...
        if(wallJumping && lastTimeOnWall < 0f){
            wallJumping = false;
        }

        if(CanWallJump() && playerMovement.LastPressedJumpTime > 0){
            Debug.Log(lastTimePlayerJump);
            wallJumping = true;
    ...
    private bool CanWallJump(){
        return (!wallJumping || (lastTimeOnWallRight > 0 && lastWallJumpDir == 1) ||
            (lastTimeOnWallLeft > 0 && lastWallJumpDir == -1)) && lastTimePlayerWallCling > 0 && !playerItemHandler.holdingItem && lastTimePlayerJump < -0.1;
    }
    ...
    public bool WallCheck(LayerMask myLayer){
        if(Physics2D.OverlapBox(frontWallCheckPoint.position, wallCheckSize, 0, myLayer)){
            return !Physics2D.OverlapBox(frontWallCheckPoint.position, wallCheckSize, 0, myLayer).isTrigger;
        }
        return false;
    }
    #endregion
    ...
    private void DetermineLaunchVector(){
        ...
        if(playerIn.x != 0){
            ...
            if(playerIn.y != 0){
                ...
                launchVector = new Vector2(playerIn.x * 0.8f, playerIn.y);
            }
