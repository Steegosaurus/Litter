...
public class ControlPointOne : MonoBehaviour
{
    ...
    void Update(){
        if(transform.parent.GetComponent<PlasticTube>().playerInTubeOne){
            if(tubeTimer < 1){
                Vector2 newPosition = Mathf.Pow(1 - tubeTimer, 3) * controlPoints[0].position +
                3 * Mathf.Pow(1 - tubeTimer, 2) * tubeTimer * controlPoints[1].position +
                3 * (1 - tubeTimer) * Mathf.Pow(tubeTimer, 2) * controlPoints[2].position +
                Mathf.Pow(tubeTimer, 3) * controlPoints[3].position;
                ...
                transform.parent.GetComponent<PlasticTube>().playerInTubeOne = false;
                if(playerI){    
    ...
    void OnTriggerStay2D(Collider2D other){
        if(other.tag == "Player" && transform.parent.GetComponent<PlasticTube>().lastTimeTravelled < 0 && !transform.parent.GetComponent<PlasticTube>().playerInTubeOne && !transform.parent.GetComponent<PlasticTube>().playerInTubeTwo){
            if(!other.GetComponent<PlayerItemHandler>().holdingItem){
                tubeTimer = 0;
                transform.parent.GetComponent<PlasticTube>().playerInTubeOne = true;
