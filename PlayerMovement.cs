...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update(){
        ...
        if(lastTimeOnGround < 0 && risingFromJump && rb.velocity.y < 0){
            risingFromJump = false;
        }
        #endregion
            ...
            if(playerInput.actions["Move"].ReadValue<Vector2>().y < 0){
                ...
                if(lastTimeOnGround < 0){
                    ...
                    if(rb.velocity.y < 0){
                        ...
                        if(playerInput.actions["Jump"].ReadValue<float>() == 1f && risingFromJump){
                            rb.gravityScale = playerData.normalGravity * playerData.risingFallGravityMult;
                        }
                        else{
                            rb.gravityScale = playerData.normalGravity * playerData.risingFallGravityMult * 1.5f;
                        }
                    }
                }
